...
export default function TimelineList({ entries }: { entries: any[] }) {
    // Group entries by UTC Month (YYYY-MM)
    const months = useMemo(() => {
        const groups: Record<string, any[]> = {};
        
        entries.forEach(entry => {
            const date = new Date(entry.date);
            // Use UTC methods to ensure strict adherence to the date string
            const key = `${date.getUTCFullYear()}-${date.getUTCMonth()}`;
            if (!groups[key]) groups[key] = [];
            groups[key].push(entry);
        });

        // Sort keys to ensure reverse chronological order (Newest first)
    ...
    return (
        ...
        >
            {months.map(([key, groupEntries]) => {
                ...
                // Force UTC for the header label as well
